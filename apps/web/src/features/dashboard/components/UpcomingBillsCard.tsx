...
export const UpcomingBillsCard = ({ bills = PLACEHOLDER_BILLS }: Props) => {
  ...
  return (
    <div className="border-[--border-color] bg-[--surface] rounded-[20px] border p-[22px]">
      <div className="mb-4 flex items-center gap-[10px]">
        <h3 className="font-display text-[17px] font-bold tracking-[-0.01em] text-[--text]">Próximas contas a pagar</h3>
        <span className="font-mono text-[11px] text-[--text-sub]">{bills.length} pendentes</span>
        <Link href="/recurring" className="ml-auto font-mono text-[11px] tracking-[0.1em] uppercase text-[--brand-accent] hover:underline">
          Ver todas
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
        {bills.map((bill) => (
          <div
            key={bill.id}
            className="border-[--border-color] bg-[--surface2] flex flex-col gap-2 rounded-[13px] border p-3"
          >
            ...
            <p className="font-mono font-semibold text-[--text] text-[13px]">
      ...
      <div className="mt-4 flex items-center justify-between border-t pt-3" style={{ borderColor: "var(--border-color)" }}>
        <span className="font-mono text-[11px] tracking-[0.1em] uppercase text-[--text-sub]">Total comprometido</span>
        <span className="font-mono font-semibold text-[--text] text-[14px]">
